var IndexPage = Ext.extend(Ext.Viewport, {
	top : {
            ...
            html: '<table width="100%" height="100%" border="0" cellspacing="0" cellpadding="0" style="table-layout:fixed;">'+
				  '<tr><td width="254px" style="background:url(res/img/top_left.jpg)">&nbsp;</td>'+
				  '<td style="background:url(res/img/top_center.jpg)">&nbsp;</td>'+
				  '<td width="544px" style="background:url(res/img/top_right.jpg)"><div id="clock" class="clock"></div></td>'+
				  '</tr>'+
				'</table>',
            margins: '5 5 5 5'
    },
	...
	center:{
			...
			margins: '0 5 5 0',
			border: false,
			html : '<iframe id="main" src="'+path+'/view/person/personList.jsp" width="100%" height="100%" scrolling="auto" frameborder="0" ></iframe>'
	},
	south : {
			region : 'south',
			height : 30,
			tbar : [{
						...
						listeners : {
							...
							select : function(d, b, c) {
								if (d.value != "") {
									var xthemem = "${path}/res/ext/resources/css/"+ d.value + ".css";
									App.setCookie("mystock-xtheme",xthemem,7);
									Ext.util.CSS.swapStyleSheet("theme", xthemem);
									var iframe = document.getElementById("main");
									iframe.src = "${path}/person/personList.jsp";
								}
							}
						}
					}]
